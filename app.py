...
# 1. Загрузка файла (пока используем локальный Excel для теста)
# Позже мы заменим эту ссылку на прямую ссылку из Google Таблиц
file_path = "Активность БПЛА.xlsx"

try:
    # Считываем все названия листов (дат) из файла
...
# 2. Создаем выпадающее меню на сайте для выбора даты
selected_date = st.selectbox("📅 Выберите дату для отображения:", sheet_names)

# 3. Читаем данные только из выбранной вкладки
df = pd.read_excel(xls, sheet_name=selected_date, header=None)
raw_data = df.values.tolist()

# Проверка, что на листе есть данные
if len(raw_data) < 3:
...
# --- Отрисовка интерфейса и графика ---
if len(df_cleaned) == 0:
    st.info("В выбранный день активность не зафиксирована.")
else:
    st.success(f"Общее количество зафиксированных меток за {selected_date}: **{df_cleaned['Количество'].sum()}**")
